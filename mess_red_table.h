#ifndef _MESS_RED_TABLE_H_
#define _MESS_RED_TABLE_H_
...
#include <rte_udp.h>
#include <rte_thash.h>
#include "mess_pkt_helper.h"
#include "mess_common.h"
...
struct mess_ft*
mess_ft_create(int cnt, int entry_size);

int
mess_ft_add_pkt(struct mess_ft *table, struct rte_mbuf *pkt, char **data);

int
mess_ft_lookup_pkt(struct mess_ft *table, struct rte_mbuf *pkt, char **data);

int32_t
mess_ft_remove_pkt(struct mess_ft *table, struct rte_mbuf *pkt);

int
mess_ft_add_key(struct mess_ft* table, struct mess_ft_ipv4_5tuple *key, char** data);

int
mess_ft_lookup_key(struct mess_ft* table, struct mess_ft_ipv4_5tuple *key, char** data);

int32_t
mess_ft_remove_key(struct mess_ft *table, struct mess_ft_ipv4_5tuple *key);

int32_t
mess_ft_iterate(struct mess_ft *table, const void **key, void **data, uint32_t *next);

void
mess_ft_free(struct mess_ft *table);

/* TODO(@sdnfv): Add function to calculate hash and then make lookup/get
 * have versions with precomputed hash values */
// hash_sig_t
// mess_ft_hash(struct rte_mbuf *buf);

static inline void
_mess_ft_print_key(struct mess_ft_ipv4_5tuple *key) {
...
static inline char*
mess_ft_get_data(struct mess_ft* table, int32_t index) {
        return &table->data[index*table->entry_size];
}

static inline int
mess_ft_fill_key(struct mess_ft_ipv4_5tuple *key, struct rte_mbuf *pkt) {
        struct ipv4_hdr *ipv4_hdr;
        struct tcp_hdr *tcp_hdr;
        struct udp_hdr *udp_hdr;

        if (unlikely(!mess_pkt_is_ipv4(pkt))) {
                return -EPROTONOSUPPORT;
        }
        ipv4_hdr = mess_pkt_ipv4_hdr(pkt);
        memset(key, 0, sizeof(struct mess_ft_ipv4_5tuple));
        key->proto  = ipv4_hdr->next_proto_id;
        key->src_addr = ipv4_hdr->src_addr;
        key->dst_addr = ipv4_hdr->dst_addr;
        if (key->proto == IP_PROTOCOL_TCP) {
                tcp_hdr = mess_pkt_tcp_hdr(pkt);
                key->src_port = tcp_hdr->src_port;
                key->dst_port = tcp_hdr->dst_port;
        } else if (key->proto == IP_PROTOCOL_UDP) {
                udp_hdr = mess_pkt_udp_hdr(pkt);
...
static inline int
mess_ft_fill_key_symmetric(struct mess_ft_ipv4_5tuple *key, struct rte_mbuf *pkt) {
        if (mess_ft_fill_key(key, pkt) < 0) {
                return -EPROTONOSUPPORT;
        }
...
mess_ft_ipv4_hash_crc(const void *data, __rte_unused uint32_t data_len,
        uint32_t init_val)
{
...
static inline uint32_t
mess_softrss(struct mess_ft_ipv4_5tuple *key)
{

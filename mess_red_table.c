...
mess_ft_create(int cnt, int entry_size) {
        ...
        struct rte_hash_parameters ipv4_hash_params = {
            .name = NULL,
            .entries = cnt,
            .key_len = sizeof(struct mess_ft_ipv4_5tuple),
            .hash_func = NULL,
        ...
        snprintf(s, sizeof(s), "mess_ft_%d-%"PRIu64, rte_lcore_id(), rte_get_tsc_cycles());
        hash = rte_hash_create(&ipv4_hash_params);
        if (hash == NULL) {
                return NULL;
        }
	ft = (struct mess_ft*)rte_calloc("table", 1, sizeof(struct mess_ft), 0);
        if (ft == NULL) {
                rte_hash_free(hash);
                return NULL;
        }
...
mess_ft_add_pkt(struct mess_ft* table, struct rte_mbuf *pkt, char** data) {
        int32_t tbl_index;
        struct mess_ft_ipv4_5tuple key;
        int err;

        err = mess_ft_fill_key(&key, pkt);
        if (err < 0) {
                return err;
        }
